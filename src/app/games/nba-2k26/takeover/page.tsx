...
import { TakeoverAccordionItem } from '@/components/games/TakeoverAccordion';
          ...
          {filteredTakeovers.length > 0 ? (
            <div className="columns-1 md:columns-2 xl:columns-3 gap-4 space-y-4">
              {filteredTakeovers.map((takeover) => (
                <div
                  key={takeover.name}
                  className="bg-background-secondary rounded-lg border border-accent/20 overflow-hidden break-inside-avoid">
                  <TakeoverAccordionItem takeover={takeover} />
                </div>
              ))}
            </div>
          ) : (
